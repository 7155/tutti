...
type connectorInvocationGate struct {
	mu      sync.Mutex
	waiters int
	slot    chan struct{}
}
...
func (broker *ConnectorBroker) skills(ctx context.Context, identity agentprincipalbiz.SessionIdentity, connectorKey string) (cliservice.CommandOutput, error) {
	...
	if err != nil {
		return cliservice.CommandOutput{}, cliservice.ServiceUnavailableError("load Connector Skills", err)
	}
	items := make([]any, 0, len(skills))
	for _, skill := range skills {
		items = append(items, map[string]any{"name": skill.Name, "title": skill.Title, "description": skill.Description, "path": skill.Path})
	}
...
func (broker *ConnectorBroker) readSkill(ctx context.Context, identity agentprincipalbiz.SessionIdentity, connectorKey, skillName string) (cliservice.CommandOutput, error) {
	...
	for _, skill := range skills {
		...
		if err != nil {
			return cliservice.CommandOutput{}, cliservice.ServiceUnavailableError("read Connector Skill", err)
		}
		return jsonValue(map[string]any{"connectorKey": connectorKey, "name": skill.Name, "title": skill.Title, "description": skill.Description, "path": skill.Path, "content": string(content)}), nil
	}
...
func (broker *ConnectorBroker) invokeConnector(ctx context.Context, identity agentprincipalbiz.SessionIdentity, request cliservice.InvokeRequest) (cliservice.CommandOutput, error) {
	...
	return broker.commands.invokeInternal(ctx, connectorKey, capabilityName, cliservice.InvokeRequest{
		Input: input, OutputMode: request.OutputMode, Context: request.Context,
	})
}
...
func (broker *ConnectorBroker) acquireInvocation(ctx context.Context, connectorKey string) (func(), error) {
	...
	if gate == nil {
		gate = &connectorInvocationGate{slot: make(chan struct{}, 1)}
		gate.slot <- struct{}{}
		broker.gates[connectorKey] = gate
	}
	broker.gatesMu.Unlock()
	gate.mu.Lock()
	if gate.waiters >= connectorInvocationQueueLimit {
		gate.mu.Unlock()
...
func (broker *ConnectorBroker) authorizedRoute(ctx context.Context, identity agentprincipalbiz.SessionIdentity, connectorKey string) (connectorBrokerRoute, error) {
	...
	return connectorBrokerRoute{}, cliservice.ServiceUnavailableError("Connector runtime is not active", nil)
}
