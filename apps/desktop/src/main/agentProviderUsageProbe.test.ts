...
test("listDesktopWorkspaceAgentProbes rejects Kimi API-key mode without an API key", async () => {
  ...
  const directory = await mkdtemp(join(tmpdir(), "tutti-kimi-missing-key-"));
  try {
    ...
    assert.match(provider?.lastError?.message ?? "", /no API key/u);
  } finally {
...
test("listDesktopWorkspaceAgentProbes reads managed Kimi config from KIMI_SHARE_DIR", async () => {
  const previousKimiCodeHome = process.env.KIMI_CODE_HOME;
  const previousKimiShareDirectory = process.env.KIMI_SHARE_DIR;
  const previousKimiModelName = process.env.KIMI_MODEL_NAME;
  const previousKimiModelAPIKey = process.env.KIMI_MODEL_API_KEY;
  const directory = await mkdtemp(join(tmpdir(), "tutti-managed-kimi-"));
  try {
    delete process.env.KIMI_CODE_HOME;
    process.env.KIMI_SHARE_DIR = directory;
    delete process.env.KIMI_MODEL_NAME;
    delete process.env.KIMI_MODEL_API_KEY;
    await writeFile(
      join(directory, "config.toml"),
    ...
    restoreOptionalEnv("KIMI_CODE_HOME", previousKimiCodeHome);
    restoreOptionalEnv("KIMI_SHARE_DIR", previousKimiShareDirectory);
    restoreOptionalEnv("KIMI_MODEL_NAME", previousKimiModelName);
    restoreOptionalEnv("KIMI_MODEL_API_KEY", previousKimiModelAPIKey);
    await rm(directory, { force: true, recursive: true });
  }
...
test("listDesktopWorkspaceAgentProbes reports a missing Kimi Coding Plan", async () => {
  const previousKimiCodeHome = process.env.KIMI_CODE_HOME;
  const previousKimiModelName = process.env.KIMI_MODEL_NAME;
  const previousKimiModelAPIKey = process.env.KIMI_MODEL_API_KEY;
  const directory = await mkdtemp(join(tmpdir(), "tutti-kimi-no-plan-"));
  try {
    process.env.KIMI_CODE_HOME = directory;
    delete process.env.KIMI_MODEL_NAME;
    delete process.env.KIMI_MODEL_API_KEY;
    await writeKimiCodingPlanFixture(directory);
    setOutboundFetcherForTesting(async () => new Response("", { status: 402 }));
    ...
    const provider = result.providers[0];
    assert.equal(provider?.availability.status, "available");
    assert.equal(provider?.lastError?.code, "subscription_required");
    assert.equal(provider?.usage, undefined);
  } finally {
    restoreOptionalEnv("KIMI_CODE_HOME", previousKimiCodeHome);
    restoreOptionalEnv("KIMI_MODEL_NAME", previousKimiModelName);
    restoreOptionalEnv("KIMI_MODEL_API_KEY", previousKimiModelAPIKey);
...
test("listDesktopWorkspaceAgentProbes reports exhausted Kimi Coding Plan quota", async () => {
  const previousKimiCodeHome = process.env.KIMI_CODE_HOME;
  const previousKimiModelName = process.env.KIMI_MODEL_NAME;
  const previousKimiModelAPIKey = process.env.KIMI_MODEL_API_KEY;
  const directory = await mkdtemp(join(tmpdir(), "tutti-kimi-exhausted-"));
  try {
    process.env.KIMI_CODE_HOME = directory;
    delete process.env.KIMI_MODEL_NAME;
    delete process.env.KIMI_MODEL_API_KEY;
    ...
    const provider = result.providers[0];
    assert.equal(provider?.availability.status, "available");
    assert.equal(provider?.lastError?.code, "quota_exhausted");
...
async function writeKimiCodingPlanFixture(directory: string): Promise<void> {
  ...
  await writeFile(
    ...
    JSON.stringify({ access_token: "kimi-access-token-1" })
  );
}
