export interface KimiConfigProjection {
  ...
  providersWithApiKeys: Set<string>;
...
/**
 * Projects only the Kimi config fields needed to determine the active billing
 * mode. API-key values are reduced immediately to presence bits and are never
...
export function projectKimiConfig(content: string): KimiConfigProjection {
  ...
  for (const rawLine of content.split(/\r?\n/u)) {
    ...
    if (section.length === 2 && assignment.key === "api_key" && value.trim()) {
      projection.providersWithApiKeys.add(provider);
    } else if (section.length === 2 && assignment.key === "base_url") {
